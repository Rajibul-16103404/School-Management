    ...
    if (empty($name_bn) || empty($name_en) || empty($eiin)) {
        ...
        try {
            ...
            if (isset($_FILES['logo']) && $_FILES['logo']['error'] !== UPLOAD_ERR_NO_FILE) {
                try {
                    $uploaded = upload_file($_FILES['logo'], 'photos', ['jpg', 'jpeg', 'png'], 2097152); // Max 2MB
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['add_doc'])) {
    ...
    if (empty($permission_date) || empty($recognition_date) || empty($recognition_number) || empty($authority_bn)) {
        ...
        try {
            // Upload PDF
            $doc_path = upload_file($_FILES['doc_file'], 'documents', ['pdf'], 5242880); // Limit to 5MB
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['add_gallery_photo'])) {
    if (!isset($_FILES['gallery_file']) || $_FILES['gallery_file']['error'] === UPLOAD_ERR_NO_FILE) {
        ...
        try {
            $photo_path = upload_file($_FILES['gallery_file'], 'photos', ['jpg', 'jpeg', 'png'], 3145728); // Max 3MB
...
<!-- Settings Layout: School Profile Info -->
<div class="admin-card">
    <div class="admin-card-header">
        <span class="admin-card-title"><i class="fa fa-school" style="color:var(--accent);"></i> প্রতিষ্ঠানের সাধারণ তথ্যাবলী</span>
    </div>
    ...
    <form method="POST" enctype="multipart/form-data">
        <div class="form-grid">
            ...
            <div class="admin-form-group form-group-full" style="display:flex; align-items:center; gap:20px; flex-wrap:wrap;">
                ...
                <?php endif; ?>
                <div style="flex:1;">
                    <label for="logo">প্রতিষ্ঠানের লোগো আপলোড (অনূর্ধ্ব ২ মেগাবাইট, JPG/PNG)</label>
...
<!-- Recognition & Permission Documents Section -->
<div class="admin-card">
    <div class="admin-card-header">
        <span class="admin-card-title"><i class="fa fa-stamp" style="color:var(--accent);"></i> পাঠদানের অনুমতি ও স্বীকৃতি নথিপত্র ব্যবস্থাপনা</span>
    </div>
    ...
    <form method="POST" enctype="multipart/form-data" style="background-color: #f8fafc; border: 1px solid var(--border-color); padding: 20px; border-radius: 8px; margin-bottom: 25px;">
        ...
        <div class="form-grid">
            ...
            <div class="admin-form-group">
                <label for="doc_file">অনুমোদনের মূল ফাইল (শুধুমাত্র PDF, সর্বোচ্চ ৫ মেগাবাইট) <span style="color:var(--danger);">*</span></label>
...
<div class="admin-card">
    <div class="admin-card-header">
        <span class="admin-card-title"><i class="fa fa-images" style="color:var(--accent);"></i> প্রতিষ্ঠানের ছবি গ্যালারি ব্যবস্থাপনা (Gallery Management)</span>
    </div>
    ...
    <form method="POST" enctype="multipart/form-data" style="background-color: #f8fafc; border: 1px solid var(--border-color); padding: 20px; border-radius: 8px; margin-bottom: 25px;">
        ...
        <div style="display:flex; align-items:center; gap:15px; flex-wrap:wrap;">
            <div style="flex:1; min-width:250px;">
                ...
                <small style="color:var(--text-muted); display:block; margin-top:5px;">অনূর্ধ্ব ৩ মেগাবাইট, JPG/PNG ফর্ম্যাট।</small>
            </div>
            <div>
            ...
            <?php foreach ($gallery_photos as $photo): ?>
                ...
                <div style="border: 1px solid var(--border-color); border-radius: 8px; overflow: hidden; background: white; text-align: center; display: flex; flex-direction: column;">
                    ...
                    <div style="padding: 8px; border-top: 1px solid var(--border-color); background: #f8fafc;">
                        <a href="?delete_photo=<?php echo urlencode($basename); ?>" class="btn-admin btn-danger" style="padding: 4px 8px; font-size: 11px; width: 100%; display: flex; justify-content: center; align-items: center; gap: 4px;" onclick="return confirm('আপনি কি নিশ্চিতভাবে এই ছবিটি গ্যালারি থেকে মুছে ফেলতে চান?');">
